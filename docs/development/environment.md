...
| ----------------------------- | ----------------------- | --------------: | ------------------------------------------------------------------------ |
| `APP_ENV`                     | public runtime          |             yes | `development`, `test` or `production`                                    |
| `DJANGO_SETTINGS_MODULE`      | public runtime          |             yes | selected settings module                                                 |
| `DJANGO_DEBUG`                | public runtime          |             yes | exact `true`/`false`; must be false in production                        |
...
| `SITE_BASE_URL`               | public runtime          |             yes | absolute URL; HTTPS in production                                        |
...
| `DATABASE_URL`                | secret                  |             yes | PostgreSQL URL; in-memory SQLite only for tests; production requires TLS |
| `DATABASE_POOL_MAX`           | public runtime          |             yes | positive integer applied to the psycopg connection pool                  |
| `AWS_REGION`                  | public runtime          |             yes | selected deployment/data region                                          |
| `AWS_ACCESS_KEY_ID`           | secret                  | local/test only | required as an emulator pair; rejected in production                     |
| `AWS_SECRET_ACCESS_KEY`       | secret                  | local/test only | required as an emulator pair; rejected in production                     |
...
| `CONTACT_DELIVERY_MODE`       | feature control         |             yes | `disabled`, `console` or `ses`; starts disabled                          |
...
1. `APP_ENV` is exactly `development`, `test` or `production`, and its Django
   settings module must match.
2. Example secrets, debug mode, wildcard/site-mismatched hosts, non-HTTPS
   ...
   without a TLS `sslmode` are rejected in production.
3. In-memory SQLite is accepted only for tests. Development and production use
   PostgreSQL.
4. The three syntactically valid storage buckets must be distinct. S3/SQS
   endpoints are credential-free absolute origins; the queue URL includes a
   queue path.
5. The default language must be a supported BCP 47 tag; lists contain no
   duplicates.
6. Optional integrations default to disabled and feature/log values are closed
   enums.
7. No secret is exposed through browser-prefixed variables.
8. Production startup fails closed when required configuration is missing or
   unsafe.
...
boundary. Adapter behavior remains inactive until its feature module and
approval evidence are implemented; parsing a variable does not claim that the
feature is active.
